...
.puser{display:grid;grid-template-columns:minmax(150px,200px) 1fr auto;align-items:center;gap:10px;border:1px solid var(--line);border-radius:4px;padding:8px 10px;margin-bottom:7px;background:#0e1116}
.puser b{font-size:12px}.pmeta{font-size:11px;color:var(--dim);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.ptag{display:inline-block;margin-left:6px;padding:0 6px;font-size:10px;line-height:16px;color:var(--accent);border:1px solid rgba(74,158,218,.4);border-radius:3px;background:rgba(74,158,218,.08)}
...
.bindfields{display:grid;grid-template-columns:repeat(2,minmax(130px,1fr));gap:10px;align-items:end}
...
.passrow{display:flex;align-items:center;gap:4px}.passrow input{flex:1;min-width:0}.passrow button{flex:0 0 auto}
...
<header>
  ...
  <nav class="links">
    <a href="https://github.com/hugeww/fanout" target="_blank" rel="noopener">GitHub</a>
  </nav>
</header>
...
<main>
  ...
  <section class="unified" id="unifiedEntry">
    <div class="utop">
      ...
      <span class="spacer"></span>
      <button id="unifiedCfg">配置入口</button>
    </div>
    <div class="udetail" id="unifiedDetail" hidden>
      <div class="uform">
        <label class="ef"><span>统一入口端口</span><input id="unifiedPort" type="text" readonly></label>
        <div class="uhint" id="unifiedHint">同一端口同时支持 SOCKS5、HTTP CONNECT 与 HTTPS 代理。取验证码/IMAP 若选「HTTPS 代理」请填 https://，选「HTTP 代理」请填 http://。所有用户共用一个端口，按用户名密码路由到各自专属的 Tunnel。</div>
      </div>
      <div class="proxyusers" id="proxyUsers"></div>
    </div>
  </section>

<div class="modal" id="unifiedcfg">
  <div class="sheet">
    <div class="head">
      <h2>统一入口配置</h2>
      <span class="spacer"></span>
      <button class="icon" data-close="unifiedcfg" title="关闭">
        <svg viewBox="0 0 24 24"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
      </button>
    </div>
    <div class="body">
      <label class="f"><span>启用统一入口</span>
        <select id="unifiedCfgEnabled">
          <option value="unified">启用</option>
          <option value="per-tunnel">停用</option>
        </select></label>
      <div class="hint">启用后同一端口同时接受 SOCKS5、HTTP CONNECT 与 HTTPS 代理。停用时保留各 Tunnel 独立入口。</div>
      ...
      <div class="hint" id="unifiedCfgListenHint" hidden></div>
      ...
      <div class="actions">
        <button data-close="unifiedcfg">取消</button>
        <button id="unifiedCfgSave" class="primary">保存</button>
      </div>
    </div>
  </div>
</div>
...
const ICON = {
  ...
  dice:'<svg viewBox="0 0 24 24"><rect x="4" y="4" width="16" height="16" rx="3"/><circle cx="9" cy="9" r="1.3"/><circle cx="15" cy="9" r="1.3"/><circle cx="9" cy="15" r="1.3"/><circle cx="15" cy="15" r="1.3"/></svg>',
...
let proxyConfig={mode:'per-tunnel',port:0,listen_addr:'0.0.0.0',users:[]},editingProxyUser='';function proxyExit(slot){return view.exits.find(function(e){return e.slot===slot;});}function proxyOwners(skip){const o={};(proxyConfig.users||[]).forEach(function(u){if(u.user!==skip)(u.tunnel_slots||[]).forEach(function(s){o[s]=u.user;});});return o;}function proxySlotText(slot){const e=proxyExit(slot);return e?(e.region||'Tunnel')+' #'+slot+' · '+(e.exit_ip||e.status):'Tunnel #'+slot;}function proxyStrategyText(u){const m={'round-robin':'轮询','random':'随机','time':'按时段'};return m[u.strategy||'round-robin']||u.strategy;}function loopbackUnified(){return proxyConfig.mode==='unified'&&(proxyConfig.listen_addr||'')==='127.0.0.1';}function renderProxy(){const enabled=proxyConfig.mode==='unified',lb=loopbackUnified();$('#unifiedDetail').hidden=false;$('#unifiedPort').value=enabled?(proxyConfig.port?('端口 '+(proxyConfig.port)):'端口 已启用（自动分配）'):'未启用';const dot=$('#unifiedDot');dot.className='dot '+(enabled?'up':'');$('#unifiedStatus').textContent=enabled?('端口 '+(proxyConfig.port||'-')+' · SOCKS5/HTTP/HTTPS'+(lb?' · 免认证':'')):'未启用（保留各 Tunnel 独立入口）';$('#unifiedHint').textContent=lb?'仅本机监听时入口免认证：SSH 本地转发（ssh -L 1080:127.0.0.1:<端口>）就是认证边界，自动汇聚所有 Tunnel 轮询出网，无需用户名密码。同一端口同时支持 SOCKS5、HTTP CONNECT 与 HTTPS 代理（先对入口 TLS）。仍监听外网时则必须用户名密码路由。':'同一端口同时支持 SOCKS5、HTTP CONNECT 与 HTTPS 代理（先对入口 TLS）。所有用户共用一个端口，按用户名密码路由到各自专属的 Tunnel；同一 Tunnel 只能属于一个用户，请求失败不会回退到本机网络。';renderProxyUsers();}function updateUnifiedListenHint(){const hint=$('#unifiedCfgListenHint');if($('#unifiedCfgListen').value==='127.0.0.1'){hint.hidden=false;hint.textContent='仅本机监听时入口免认证：SSH 本地转发（ssh -L 1080:127.0.0.1:<端口>）就是认证边界，自动汇聚所有 Tunnel 轮询出网，无需用户名密码。同一端口同时支持 SOCKS5、HTTP CONNECT 与 HTTPS 代理。仍监听外网时则必须用户名密码路由。';}else{hint.hidden=true;hint.textContent='';}}function openUnifiedCfg(){$('#unifiedCfgEnabled').value=proxyConfig.mode==='unified'?'unified':'per-tunnel';$('#unifiedCfgListen').value=proxyConfig.listen_addr||'0.0.0.0';$('#unifiedCfgPort').value=proxyConfig.port||'';$('#unifiedCfgHint').textContent='';updateUnifiedListenHint();openModal('unifiedcfg');}function renderProxyUsers(){const box=$('#proxyUsers');if(loopbackUnified()){let html='<div class="hint">仅本机监听免认证模式：无需用户名密码，自动汇聚所有在线 Tunnel 轮询出网。同一端口同时支持 SOCKS5、HTTP CONNECT 与 HTTPS 代理。切回外网监听（0.0.0.0）后才需要“用户与专属 Tunnel 绑定”。</div>';if(proxyConfig.port){const socks=socksURL('127.0.0.1',proxyConfig.port),httpu=httpURL('127.0.0.1',proxyConfig.port),httpsu=httpsURL('127.0.0.1',proxyConfig.port);html+='<div class="share" style="margin-top:10px">'+esc(socks)+'<br>'+esc(httpu)+'<br>'+esc(httpsu)+'</div><button id="proxyCopyLoopback" style="margin-top:8px">复制地址</button>';}box.innerHTML=html;editingProxyUser=null;return;}const users=proxyConfig.users||[];let out='<div class="ptop"><h3>用户与专属 Tunnel 绑定</h3><span class="count">'+users.length+' 个用户</span><span class="spacer"></span><button id="proxyAdd">新建用户</button></div>';if(!users.length)out+='<div class="hint">暂无统一入口用户。Tunnel 是独占的，只能归属一个用户，不能共享。</div>';else out+=users.map(function(u){const slots=(u.tunnel_slots||[]).map(proxySlotText).join('、');return '<div class="puser"><b>'+esc(u.user)+'</b>'+(u.tag?'<span class="ptag">'+esc(u.tag)+'</span>':'')+'<span class="pmeta">'+esc(proxyStrategyText(u))+' · '+esc(slots)+'</span><span><button class="icon" data-proxy-copy="'+esc(u.user)+'" title="复制 SOCKS5 / HTTP / HTTPS 访问凭据">'+ICON.copy+'</button><button class="icon" data-proxy-edit="'+esc(u.user)+'" title="编辑">编辑</button><button class="icon danger" data-proxy-del="'+esc(u.user)+'" title="删除">'+ICON.trash+'</button></span></div>';}).join('');if(editingProxyUser!==null){const old=users.find(function(u){return u.user===editingProxyUser;})||{user:'',pass:'',tunnel_slots:[],strategy:'round-robin',interval_seconds:60};const owner=proxyOwners(old.user),slots=old.tunnel_slots||[];const picks=view.exits.map(function(e){const used=owner[e.slot],checked=slots.indexOf(e.slot)>=0,off=!!used&&!checked;return '<label class="'+(off?'off':'')+'"><input type="checkbox" class="proxy-slot" value="'+e.slot+'"'+(checked?' checked':'')+(off?' disabled':'')+'><span>'+esc(proxySlotText(e.slot))+'</span>'+(used?'<small>归属 '+esc(used)+'</small>':'')+'</label>';}).join('');out+='<div class="bindform"><div class="bindtop"><b>'+ (old.user?'编辑用户 '+esc(old.user):'新建用户') +'</b><span class="spacer"></span><button class="icon" id="proxyCancel" title="取消">'+ICON.x+'</button></div><div class="bindfields"><label class="ef"><span>用户名</span><span class="passrow"><input id="proxyUser" type="text" value="'+esc(old.user)+'"'+(old.user?' readonly':'')+'><button type="button" class="icon" id="proxyGenUser" title="随机生成用户名"'+((old.user?' disabled':'') )+'>'+ICON.dice+'</button></span></label><label class="ef"><span>密码</span><span class="passrow"><input id="proxyPass" type="password" value="'+esc(old.pass)+'"><button type="button" class="icon" id="proxyGenPass" title="随机生成密码">'+ICON.dice+'</button></span></label><label class="ef"><span>标记</span><input id="proxyTag" type="text" maxlength="16" value="'+esc(old.tag||'')+'" placeholder="如 JP"></label><label class="ef"><span>调度策略</span><select id="proxyStrategy"><option value="round-robin">每次请求轮询</option><option value="random">随机</option><option value="time">按时段</option></select></label></div><label class="ef" id="proxyIntervalWrap" style="margin-top:10px"'+((old.strategy||'round-robin')==='time'?'':' hidden')+'><span>切换间隔（秒）</span><input id="proxyInterval" type="text" inputmode="numeric" value="'+(old.interval_seconds||60)+'"></label><div class="tunnelpick">'+(picks||'<span class="hint">请先创建并启动 Tunnel。</span>')+'</div><div class="bindfoot"><span class="conflict" id="proxyConflict"></span><span class="spacer"></span><button id="proxyUserSave" class="primary">保存用户</button></div></div>';}box.innerHTML=out;if(editingProxyUser!==null){$('#proxyStrategy').value=old.strategy||'round-robin';}}async function loadProxyConfig(){try{proxyConfig=await api('/api/proxy');renderProxy();}catch(e){$('#unifiedStatus').textContent='读取统一入口配置失败：'+e.message;}}async function saveUnified(btn){const port=$('#unifiedCfgPort').value.trim();const mode=$('#unifiedCfgEnabled').value;const cfg=Object.assign({},proxyConfig,{mode:mode,listen_addr:$('#unifiedCfgListen').value,port:port?parseInt(port,10):0});if(btn)btn.disabled=true;try{proxyConfig=await api('/api/proxy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(cfg)});renderProxy();closeModal('unifiedcfg');toast(mode==='unified'?('已启用统一入口'+(cfg.listen_addr==='127.0.0.1'?'（仅本机免认证）':'')):'已关闭统一入口（保留各 Tunnel 独立入口）');}catch(err){$('#unifiedCfgHint').textContent=err.message;}if(btn)btn.disabled=false;}$('#unifiedCfg').onclick=function(){ openUnifiedCfg(); };$('#unifiedCfgSave').onclick=function(e){ saveUnified(this); };$('#unifiedCfgListen').onchange=updateUnifiedListenHint;document.addEventListener('click',async function(e){const add=e.target.closest('#proxyAdd'),edit=e.target.closest('[data-proxy-edit]'),del=e.target.closest('[data-proxy-del]'),copyBtn=e.target.closest('[data-proxy-copy]'),copyLb=e.target.closest('#proxyCopyLoopback'),cancel=e.target.closest('#proxyCancel'),save=e.target.closest('#proxyUserSave');if(copyLb){if(!proxyConfig.port){toast('请先启用统一入口并设置端口',true);return;}copy(socksURL('127.0.0.1',proxyConfig.port)+'\n'+httpURL('127.0.0.1',proxyConfig.port)+'\n'+httpsURL('127.0.0.1',proxyConfig.port));return;}if(copyBtn){const u=proxyConfig.users.find(function(x){return x.user===copyBtn.dataset.proxyCopy;});if(!u){toast('用户不存在',true);return;}if(proxyConfig.mode!=='unified'||!proxyConfig.port){toast('请先启用统一入口并设置端口',true);return;}const host=view.public_ip||location.hostname;copy(socksURL(host,proxyConfig.port,u.user,u.pass)+'\n'+httpURL(host,proxyConfig.port,u.user,u.pass)+'\n'+httpsURL(host,proxyConfig.port,u.user,u.pass));return;}if(add){editingProxyUser='';renderProxyUsers();return;}if(edit){editingProxyUser=edit.dataset.proxyEdit;renderProxyUsers();return;}if(cancel){editingProxyUser=null;renderProxyUsers();return;}if(del){const user=del.dataset.proxyDel;if(!confirm('确认删除用户 '+user+'？其绑定的 Tunnel 会释放'))return;try{proxyConfig=await api('/api/proxy/user?user='+encodeURIComponent(user),{method:'DELETE'});if(editingProxyUser===user)editingProxyUser=null;renderProxy();toast('已删除');}catch(err){toast(err.message,true);}return;}if(save){const user=$('#proxyUser').value.trim(),pass=$('#proxyPass').value.trim(),strategy=$('#proxyStrategy').value,slots=Array.from(document.querySelectorAll('.proxy-slot:checked')).map(function(x){return parseInt(x.value,10);});const conflict=Object.keys(proxyOwners(user)).some(function(s){return slots.indexOf(parseInt(s,10))>=0;});if(!user||!pass||!slots.length){$('#proxyConflict').textContent='用户名、密码和至少一个 Tunnel 都是必填项';return;}if(conflict){$('#proxyConflict').textContent='该 Tunnel 已被其他用户占用，不能重复分配';return;}save.disabled=true;try{proxyConfig=await api('/api/proxy/user',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user:user,pass:pass,tag:$('#proxyTag').value.trim(),tunnel_slots:slots,strategy:strategy,interval_seconds:strategy==='time'?parseInt($('#proxyInterval').value,10)||60:0})});editingProxyUser=null;renderProxy();toast('已保存');}catch(err){$('#proxyConflict').textContent=err.message;}save.disabled=false;}});document.addEventListener('click',function(e){const g=e.target.closest('#proxyGenPass')||e.target.closest('#proxyGenUser');if(g){const sets=['ABCDEFGHJKLMNPQRSTUVWXYZ','abcdefghijkmnpqrstuvwxyz','23456789','!@#$%^&*-_'],chars=sets.join('');let p='';const bytes=new Uint8Array(16);crypto.getRandomValues(bytes);for(let i=0;i<bytes.length;i++){p+=chars[bytes[i]%chars.length];}sets.forEach(function(s){p+=s[crypto.getRandomValues(new Uint8Array(1))[0]%s.length];});p=p.split('').sort(function(){return Math.random()-0.5;}).join('');if(g.id==='proxyGenPass'){$('#proxyPass').value=p;}else{const bytes2=new Uint8Array(6);crypto.getRandomValues(bytes2);let un='';const uc='abcdefghijkmnpqrstuvwxyz23456789';for(let i=0;i<bytes2.length;i++){un+=uc[bytes2[i]%uc.length];}$('#proxyUser').value='fo'+un;$('#proxyPass').value=p;}return;}});document.addEventListener('change',function(e){if(e.target&&e.target.id==='proxyStrategy'){const w=$('#proxyIntervalWrap');if(w)w.hidden=e.target.value!=='time';}});
...
function httpURL(host, port, user, pass){
  if(!user) return 'http://' + host + ':' + port;
  return 'http://' + user + ':' + pass + '@' + host + ':' + port;
}
function httpsURL(host, port, user, pass){
  if(!user) return 'https://' + host + ':' + port;
  return 'https://' + user + ':' + pass + '@' + host + ':' + port;
}
